...
export default function Feed({ departmentId, feedopen }: departmentIdProps) {
 ...
 /* useEffect(() => {
    const handlegetdepartApi = async () => {
      const result = await axios
        .post("/api/locationdetails", { customerid: customerid })
  ...
  const handleCustomerChild=(name)=>{
    dispatch(addCustomername(name));
    router.push(`/updatecustomer/${customername}`)
